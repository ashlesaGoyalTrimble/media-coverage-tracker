...
async def send_to_all_assistants(request: MessageRequest):
    ...
    try:
        ...
        print(f"Exception in send_to_all_assistants route: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                print(f"Exception parsing JSON in send_message route: {json_e}")
                return {"message": "Error parsing response"}
        else:
            print(f"Exception in send_message route: HTTP {response.status_code}")
            raise HTTPException(status_code=response.status_code, detail=f"Error: {response.text}")
    except HTTPException:
        raise
    except Exception as e:
        print(f"Exception in send_message route: {e}")
...
async def upload_image_route(assistant_id: str, session_id: str, file: UploadFile):
    ...
    try:
        ...
        print(f"Exception in upload_image_route: {e}")
...
async def trigger_process_hyperlinks(
    ...
    try:
        ...
        if not file.filename.endswith(('.xlsx', '.xls')):
            print("Exception: invalid file type")
            raise HTTPException(status_code=400, detail="File must be an Excel file (.xlsx or .xls)")
        
        print(f"Starting to process {file.filename} with sheet '{sheet_name}'")
        
        # Process the uploaded file with timeout to prevent AWS gateway timeout
        try:
            processed_file_bytes = await asyncio.wait_for(
                process_hyperlinks(file, sheet_name),
                timeout=300  # 5 minute timeout for AWS compatibility
            )
            print(f"Successfully processed {file.filename}")
        except asyncio.TimeoutError:
            print(f"Processing timeout for {file.filename} - returning partial results")
            # Try to get partial results by reading original file
            await file.seek(0)
        ...
        print(f"AWS Gateway timeout reached for {file.filename}")
        try:
            await file.seek(0)
            original_file_bytes = await file.read()
            return StreamingResponse(
                ...
                media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                headers={
                    "Content-Disposition": f"attachment; filename=timeout_{file.filename}"
                }
            )
        except Exception as timeout_fallback_e:
            print(f"Exception in timeout fallback: {timeout_fallback_e}")
            raise HTTPException(status_code=408, detail="Processing timed out")
    except Exception as e:
        error_msg = str(e)
        print(f"Exception in trigger_process_hyperlinks: {error_msg}")
        
        # Handle 504 Gateway timeout specifically
        if "504" in error_msg or "Gateway" in error_msg:
            print("Detected 504 Gateway timeout error")
            try:
                await file.seek(0)
                original_file_bytes = await file.read()
                return StreamingResponse(
                    ...
                    media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    headers={
                        "Content-Disposition": f"attachment; filename=gateway_timeout_{file.filename}"
                    }
                )
            except Exception:
                raise HTTPException(status_code=504, detail="Gateway timeout occurred")
        ...
        try:
            ...
            return StreamingResponse(
                ...
                media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                headers={
                    "Content-Disposition": f"attachment; filename=error_{file.filename}"
                }
            )
        except Exception as fallback_e:
            print(f"Exception in fallback file return: {fallback_e}")
            raise HTTPException(status_code=500, detail="Processing failed and could not return file")